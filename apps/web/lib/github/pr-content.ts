...
export function resolvePullRequestAppBaseUrl(
  ...
  // A self-hosted deployment has one public URL, so there is no preview-vs-
  // production split to pick between: the caller's value wins, otherwise the
  // configured app URL.
  const candidates = [appBaseUrl, process.env.APP_URL];
...
export interface GeneratePullRequestContentParams {
  ...
  appBaseUrl?: string;
  /**
   * Directory to read the diff from.
   *
   * A chat's worktree, since that is where its branch is checked out. Defaults
   * to the sandbox's own working directory — the session's repository — for
   * callers that are not chat-scoped.
   */
  cwd?: string;
}
...
export async function generatePullRequestContentFromSandbox(
  ...
  if (!SAFE_BRANCH_PATTERN.test(baseBranch)) {
    return {
      success: false,
      error: "That base branch can't be used. Pick a different one.",
    };
  }
  ...
  if (!SAFE_BRANCH_PATTERN.test(branchName)) {
    return {
      success: false,
      error: "That branch name can't be used. Pick a different one.",
    };
  }
  ...
  if (!SAFE_BRANCH_PATTERN.test(finalBaseRef)) {
    return {
      success: false,
      error: "That base branch doesn't look right. Pick it again.",
    };
  }
  ...
  if (!diffStats.trim() && !commitLog.trim()) {
    ...
    if (uncommittedStatus.stdout.trim()) {
      return {
        success: false,
        error:
          "Some changes couldn't be saved to the branch, so there is nothing to open a pull request from.",
      };
    }
...
${commitLog}`,
    ...
    // Fall back to a deterministic PR body when generation fails or
    // returns something that does not match the schema.
