...
export async function compressImageFile(file: File): Promise<File> {
  ...
  if (!context) {
    throw new Error("We couldn't prepare that image. Try a different one.");
  }
  ...
  return new Promise((resolve, reject) => {
    canvas.toBlob(
      (blob) => {
        if (!blob) {
          reject(
            new Error("We couldn't prepare that image. Try a different one."),
          );
          return;
        }
...
function loadImageElement(file: File): Promise<HTMLImageElement> {
  return new Promise((resolve, reject) => {
    ...
    image.addEventListener("error", () => {
      URL.revokeObjectURL(objectUrl);
      reject(new Error("We couldn't read that image. Try a different one."));
    });
