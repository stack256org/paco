...
const DOWNLOAD_FAILED =
  "We couldn't build the file of your changes. Reload the page and try again.";
...
async function getTrackedDiff(params: {
  ...
  if (!result.success) {
    ...
    console.error("[download-diff] git diff failed:", stderr);
    throw new DownloadDiffError(DOWNLOAD_FAILED, 400);
  }
...
async function getUntrackedDiff(
  ...
  if (!result.success) {
    ...
    console.error("[download-diff] git ls-files failed:", stderr);
    throw new DownloadDiffError(DOWNLOAD_FAILED, 400);
  }
...
export async function createDownloadDiff(
  sandbox: Sandbox,
  /** The chat's worktree — the only place its changes exist. */
  cwd: string,
