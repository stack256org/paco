 ...
 * is threaded at all: a chat explicitly switched to a different backend
 * would otherwise have its work reviewed on Claude Code without that
 * appearing anywhere.
 */
...
export async function runReviewerGate(
   ...
   * reports `structuredOutput: false` returns free text no matter what
   * schema it is handed — `runAgentTurn` warns and hands back `undefined`.
   * Running the turn anyway would spend a full agent turn to
