/**
 * Model a new chat starts on.
 *
 * Opus, because the chat's own model is the orchestrator: it plans, researches
 * and decides, then hands implementation to the Sonnet executor and lookups to
 * the Haiku explorer. Starting the orchestrator on Sonnet put the strongest
 * model nowhere in the loop.
 */
export const APP_DEFAULT_MODEL_ID = "opus";
export const DEFAULT_CONTEXT_LIMIT = 200_000;

const TOKENS_PER_MILLION = 1_000_000;

interface BaseAvailableModel {
  id: string;
  name: string;
...
interface AvailableModelCostTier {
  input?: number;
  output?: number;
  cache_read?: number;
}
...
export type AvailableModel = BaseAvailableModel & {
