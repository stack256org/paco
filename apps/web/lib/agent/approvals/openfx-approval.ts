...
/**
 * ACP tool-call kinds that never write or run anything (PROTOCOL.md §3's
 ...
 * rules.
 *
 * `rawInput`'s shape is provider-specific and undocumented on the wire
 * (PROTOCOL.md §5), so this is deliberately conservative: a kind this
 * function cannot confidently place lands on `"Other"`, which
 * `decideApproval` treats as "a tool it does not recognise" and always asks
 * about — safe, per the same fails-closed rule `decideApproval` itself
 * documents, rather than guessing allow.
 */
function toolNameForKind(kind: string): string {
  ...
  if (WRITE_KINDS.has(kind)) {
    ...
    // If the real provider's `rawInput` doesn't use one of those keys, the
    // lookup fails closed to "ask" rather than allow — see `writeTarget`'s
    // own doc in `approval-policy.ts`.
    return "Edit";
  }
  if (kind === "execute") {
    return "Bash";
...
/**
 * PROTOCOL.md §5's option ids: `allow_once`, `allow_always`, `reject_once`.
 * Preferring the "once" variant over "always" mirrors `decideApproval`
 * itself, which is re-evaluated on every call rather than remembered.
 */
function selectDecision(
  ...
  if (!optionId) {
    // The server offered nothing usable — PROTOCOL.md §5 treats a missing
    // selection the same as an explicit cancel/deny.
...
/**
 * Build the `OpenFxBackend.onApprovalRequest` handler for one turn.
 *
 ...
 *
 * There is no HTTP round trip here, unlike Claude's hook: OpenFX's ACP
 ...
 */
export function createOpenFxApprovalHandler(params: {
  chatId: string;
  worktree: string;
}): PermissionHandler {
  return async (
