...
export function parseUsageDateRange(params: {
  ...
  if (from === null || to === null) {
    return {
      ok: false,
      error: "Pick both a start and an end date for this report.",
    };
  }
  ...
  if (!isDateOnly(from) || !isDateOnly(to)) {
    return {
      ok: false,
      error: "Those dates don't look right. Pick them again from the calendar.",
    };
  }
...
export function parsePublicUsageDate(
  ...
  if (explicitMatch) {
    ...
    if (!rangeResult.ok || !rangeResult.range) {
      return {
        ok: false,
        error: rangeResult.ok
          ? "That date range doesn't look right. Pick it again from the calendar."
          : rangeResult.error,
      };
    }
