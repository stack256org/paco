 ...
 * the module. This function takes `organizationId` and `promotedBy` as
 * plain arguments and performs no authorization of its own, so as an
 * endpoint it would let any caller write attacker-chosen content, under a
 * forged author, into memory that `load-for-turn.ts` injects into every
 * agent turn — a persistent prompt-injection write primitive.
 *
