...
export type AssistantMessageGroupsProps = {
  ...
  /** Undo this turn, restoring the worktree to the checkpoint it names. */
  onRevert?: (checkpointSha: string) => void;
  isReverting?: boolean;
  /**
...
export function AssistantMessageGroups({
  ...
  // Only offer a revert where there is both something to undo and a recorded
  // point to undo to. Turns that only read leave the worktree untouched, and
  // turns from before checkpoints existed have no restore point.
  const checkpointSha = message.metadata?.checkpointSha;
  const canRevert = Boolean(checkpointSha) && changedFiles.length > 0;
  ...
  return (
    <>
      <ToolCallsSummaryBar
        ...
        {...(canRevert && onRevert
          ? { onRevert: () => onRevert(checkpointSha as string) }
          : {})}
