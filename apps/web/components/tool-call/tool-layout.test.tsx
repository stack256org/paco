...
describe("ToolLayout interrupted state", () => {
  test("renders interrupted tool calls with yellow header styling and OctagonPause icon", () => {
    ...
    expect(html).not.toContain("border-warning/30 bg-warning/10");
...
describe("ToolLayout error state", () => {
  test("renders failed tool calls with error icon and tool name in red in minimized view", () => {
    ...
    expect(html).toContain("text-error/80");
    expect(html).toContain("bg-transparent");
    ...
    const html = renderToStaticMarkup(
      <ToolLayout
        ...
        state={{
          ...baseState,
          error:
            "Failed to read file: ENOENT: no such file or directory, stat '/workspace/nope'",
        }}
        defaultExpanded
      />,
    );
  ...
  test("renders expanded details inline without muted background", () => {
    ...
    expect(html).toContain('aria-expanded="true"');
    expect(html).toContain("bg-transparent");
    expect(html).not.toContain("bg-base-200/35");
    ...
    expect(html).not.toContain("bg-base-100/60 p-3");
    expect(html).not.toContain("border-t border-base-300 pt-3");
  });
