...
function DiffStats({
  ...
  return (
    <div className="flex items-center gap-1 font-mono text-xs">
      {added !== null ? <span className="text-success">+{added}</span> : null}
      {removed !== null ? <span className="text-error">-{removed}</span> : null}
    </div>
  );
}
...
function PrStatus({ status }: { status: "open" | "merged" | "closed" | null }) {
  ...
  if (status === "merged") {
    return (
      <div className="flex items-center gap-1 rounded-md bg-secondary/10 px-1.5 py-0.5 text-xs text-secondary">
...
function SessionGroup({
  ...
  return (
    <div>
      <h3 className="mb-2 px-3 text-xs font-medium uppercase tracking-wider text-base-content/60">
        {dateGroup}
      </h3>
      <div className="space-y-0.5">
        {sessions.map((session) => (
          <button
            ...
            className="flex w-full items-start justify-between gap-2 rounded-md px-3 py-2.5 text-left transition-colors hover:bg-base-200/50"
          >
            <div className="flex min-w-0 flex-1 items-start gap-2">
              {session.hasStreaming ? (
                <span className="mt-1 h-2 w-2 shrink-0 rounded-full bg-base-content animate-pulse" />
              ) : session.hasUnread ? (
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 text-xs text-base-content/60">
                  {session.repoName ? (
                    ...
                    <span className="text-base-content/60">Working...</span>
                  ) : null}
                </p>
              </div>
            </div>
...
function SessionDrawerSkeleton() {
  return (
    <div className="space-y-4 p-3">
      <div>
        <div className="mb-2 h-3 w-16 animate-pulse rounded bg-base-200" />
