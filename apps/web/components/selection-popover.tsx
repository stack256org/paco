...
export function SelectionPopover({
  ...
  return (
    // The popover is a container, not a control: the handlers below are for
    // dismissal and pointer tracking, and every actionable item inside it is a
    // real button with its own keyboard handling.
    ...
    <div
      ...
      className={cn(
        ...
        "rounded-xl border border-base-300 bg-base-200 shadow-lg",
      )}
      style={{
        top: `${top}px`,
        left: `${left}px`,
    ...
    >
      {state.type === "button" ? (
        ...
        <div className="flex w-80 flex-col gap-2 p-3">
          ...
          <div className="max-h-20 overflow-auto rounded-md bg-base-200/60 px-2.5 py-1.5">
            <p className="line-clamp-3 font-mono text-xs text-base-content/60">
              {state.selectedText}
            </p>
          </div>
          ...
          <textarea
            ...
            rows={2}
            className={cn(
              "w-full resize-none rounded-md border border-base-300 bg-base-100 px-2.5 py-2",
              "text-sm text-base-content placeholder:text-base-content/60",
          ...
          <div className="flex items-center justify-between">
            <button
              type="button"
              onClick={hide}
              className="flex items-center gap-1 rounded-md px-2 py-1 text-xs text-base-content/60 transition-colors hover:text-base-content"
            >
              <X className="h-3 w-3" />
              Cancel
            </button>
