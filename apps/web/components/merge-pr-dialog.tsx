...
import type { CheckRun, MergeMethod } from "@/lib/github/queries/pr";
import {
  canForceMerge,
  hasBlocker,
...
export function MergePrDialog({
  ...
  const handleForceClick = () => {
    if (forceConfirming) {
      ...
      setForceConfirming(false);
      // The "merge anyway" path used to bypass a stale-head check Paco made
      // itself. GitHub performs that check now, so this is an ordinary merge.
      void handleMerge();
    } else {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="grid gap-4 py-2">
              ...
              <div className="space-y-3 py-3 pr-3 pl-4">
                ...
                <div className="space-y-1.5 pl-6">
                  {blockingBlockers.map((blocker) => (
                    <p
                      key={blocker.code}
                      className="text-[13px] leading-snug text-base-content/60"
                    >
                      {blocker.message}
                    </p>
                  ))}
          ...
          <div className="flex items-center justify-between rounded-md border border-base-300 bg-base-200/30 p-3">
