 ...
 * - When the CLI reports a cost, the cumulative USD cost is rendered
...
export function MessageModelPill({
  ...
  // Cost goes in the tooltip so the exact figure is visible even though the
  // pill itself rounds.
  const tooltipParts: string[] = [];
  if (hasCost) {
    tooltipParts.push(
      `Cost: ${(totalMessageCost as number).toFixed(6)} (reported)`,
    );
  }

  const pill = (
    <span className="inline-flex max-w-[320px] items-center gap-1 rounded px-1.5 py-0.5 text-[11px] leading-tight text-base-content/50 transition-colors hover:text-base-content/80">
