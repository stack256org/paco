...
import type { CheckRun, CheckState } from "@/lib/github/queries/pr";
  ...
  if (failed === 0 && pending === 0) {
    ...
    return (
      <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-success">
        <Check className="h-3 w-3 text-primary-content" strokeWidth={3} />
      </span>
    );
  }
  ...
  if (passed === 0 && pending === 0) {
    ...
    return (
      <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-error">
        <X className="h-3 w-3 text-primary-content" strokeWidth={3} />
      </span>
    );
  }
...
function CheckStateIcon({
  ...
  if (state === "pending") {
    return (
      <CircleDotDashed
        className={cn("h-4 w-4 shrink-0 text-warning", className)}
      />
    );
  }
  return (
    <X
      className={cn("h-4 w-4 shrink-0 text-error", className)}
      strokeWidth={2.5}
    />
  );
}
    ...
    <div>
      <div className="flex items-center gap-1">
        <button
          ...
          className="flex min-w-0 flex-1 items-center gap-1.5 py-1 text-xs font-medium text-base-content/60 hover:text-base-content"
        >
          {open ? (
...
function FixErrorsButton({
  ...
  return (
    <button
      type="button"
      className="flex items-center gap-1 rounded px-2 py-1 text-xs text-base-content/60 hover:bg-base-200 hover:text-base-content disabled:pointer-events-none disabled:opacity-50"
  ...
  return (
    <div className="rounded-md border border-base-300 bg-base-200/40">
      ...
      <div className="flex items-center gap-2.5 p-3">
        ...
        >
          {showLoading ? (
            <Loader2 className="h-5 w-5 shrink-0 animate-spin text-base-content/60" />
          ) : (
            <HeaderStatusIcon
              passed={passed}
              failed={failed}
          ...
          <div className="flex shrink-0 items-center gap-1">
            {onRefresh && (
              <button
                ...
                className="rounded p-1 text-base-content/60 hover:bg-base-200 hover:text-base-content"
                onClick={() => onRefresh()}
              >
                <RefreshCw
        ...
        {!showLoading &&
          (detailsOpen ? (
            ...
            <ChevronRight className="h-4 w-4 shrink-0 text-base-content/60" />
          ))}
      </div>
