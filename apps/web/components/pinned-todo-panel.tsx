...
function InProgressIcon({ className }: { className?: string }) {
  return (
    ...
    >
      ...
      <ArrowRight
        className="relative h-2.5 w-2.5 text-base-200"
        strokeWidth={3}
      />
    </span>
  );
...
export function PinnedTodoPanel({ todos }: PinnedTodoPanelProps) {
  ...
  return (
    <div className="mx-4 overflow-hidden rounded-t-xl border border-b-0 border-base-300/60 bg-base-100 transition-all">
      {/* Header bar — always visible */}
      <button
        type="button"
        onClick={() => setIsMinimized((v) => !v)}
        className="flex w-full items-center gap-2 px-3 py-2 text-left transition-colors hover:bg-base-content/5"
      >
        {/* Chevron on the left */}
        {isMinimized ? (
          <ChevronRight className="h-3.5 w-3.5 shrink-0 text-base-content/50" />
        ) : (
        ...
        {isMinimized && activeTask?.content && (
          <span className="min-w-0 flex-1 truncate font-mono text-xs font-normal text-base-content/50">
            {activeTask.content}
          </span>
        )}
      </button>
      ...
      {/* Expanded todo list */}
      {!isMinimized && (
        <div className="max-h-48 overflow-y-auto border-t border-base-300/40 px-3 py-2">
                ...
                >
                  <span className="shrink-0">
                    {todo.status === "completed" ? (
                      <CompletedIcon className="text-base-content/50" />
                    ) : todo.status === "in_progress" ? (
                      <InProgressIcon className="text-base-content/50" />
                    ) : (
                      <PendingIcon className="text-base-content/30" />
                    )}
                  </span>
                  <span
                    className={cn(
