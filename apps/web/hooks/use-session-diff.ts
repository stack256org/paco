...
/**
 * The diff for one chat.
 *
 * Scoped to a chat, not a session: each chat works in its own git worktree on
 * its own branch, so a session-wide diff would span several branches and could
 * not be shown as one set of changes — or turned into one pull request.
 */
export function useSessionDiff(
  ...
  const {
    ...
    sandboxConnected
      ? `/api/sessions/${sessionId}/diff?chatId=${encodeURIComponent(chatId)}`
      : null,
    fetcher,
    {
