...
import { previewSlug } from "@/lib/preview/hostname";
import { fetcherNoStore } from "@/lib/swr";
import { ACTIVE_SESSIONS_KEY } from "@/lib/sessions/session-cache-keys";

/** Client-side chat list item. Mirrors the server's `ChatSummary`. */
...
export function useSessionChats(
  ...
  useEffect(() => {
    ...
    if (wasStreamingInSession && !summary.hasStreaming) {
      void mutateSessionSummaries(ACTIVE_SESSIONS_KEY);
    }
  }, [
    sessionId,
    summaryHasUnread,
