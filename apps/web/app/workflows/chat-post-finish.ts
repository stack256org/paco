...
/**
 * Ceiling on a persisted assistant message.
 *
 * Per-result truncation bounds any single tool output, but a long turn can still
 * accumulate hundreds of parts. Serialising an unbounded message is what
 * exhausted the heap and took the server down mid-stream, losing the reply
 * entirely — so the row is capped as a last line of defence.
 *
 * Tool outputs are shed first and oldest-first, because the model's prose and
 * the most recent results are what a returning reader actually needs.
 */
const MAX_PERSISTED_MESSAGE_BYTES = 1_000_000;

function capMessageSize<T extends { parts: unknown[] }>(message: T): T {
  const size = (value: unknown) => JSON.stringify(value)?.length ?? 0;
  if (size(message) <= MAX_PERSISTED_MESSAGE_BYTES) {
    return message;
  }

  const parts = [...message.parts];
  for (let i = 0; i < parts.length; i++) {
    const part = parts[i] as { type?: string; output?: unknown };
    if (
      ...
      parts[i] = {
        ...part,
        output: "… output dropped to keep this message storable",
      };
      if (size({ ...message, parts }) <= MAX_PERSISTED_MESSAGE_BYTES) {
        break;
      }
    }
  }
...
export async function persistAssistantMessageWithToolResults(
  ...
  try {
    const dedupedMessage = capMessageSize(dedupeMessageReasoning(message));
...
export async function clearActiveStream(
  ...
  // The loop does iterate more than once: the `return`s are on the success and
  // give-up paths, and a retryable failure falls through to the delay below.
