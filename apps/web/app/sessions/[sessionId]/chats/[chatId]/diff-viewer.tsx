...
function StatusBadge({ status }: { status: DiffFile["status"] }) {
  const styles = {
    ...
    renamed: "bg-warning/10 text-warning",
  };
...
function StagingBadge({
  ...
  const styles = {
    unstaged: "bg-warning/10 text-warning",
    ...
    <div className="border-b border-base-300 last:border-b-0">
      <button
        ...
        className={cn(
          ...
          isGenerated ? "cursor-default opacity-70" : "hover:bg-base-200/50",
        )}
      >
        {isGenerated ? (
        ...
        <div className="flex min-w-0 flex-1 items-center gap-2">
          <span className="truncate text-sm">
            {dirPath && <span className="text-base-content/60">{dirPath}</span>}
            <span className="font-medium text-base-content">{fileName}</span>
          </span>
          <StatusBadge status={file.status} />
          <StagingBadge stagingStatus={file.stagingStatus} />
          {isGenerated && (
            <span className="rounded px-1.5 py-0.5 text-[10px] font-medium uppercase text-base-content/60 bg-base-200">
              Generated
            </span>
          )}
        </div>
      ...
      {isExpanded && !isGenerated && (
        <div className="border-t border-base-300">
          {file.diff ? (
            ...
            <div className="px-4 py-3 text-xs text-base-content/60">
...
export function DiffViewer({ open, onOpenChange }: DiffViewerProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogHeader className="shrink-0 border-b border-base-300 px-4 py-3">
        ...
        {filteredFiles.length > 0 && (
          <div className="flex shrink-0 items-center justify-between border-t border-base-300 px-4 py-2 text-xs text-base-content/60">
            <span>
