...
/**
 * When the client owes the agent a reply.
 *
 * Exactly one tool is executed in the browser: `ask_user_question`. Everything
 * else runs inside the Claude Code process, which carries on by itself and
 * needs nothing submitted back. So the only reason to send automatically is
 * that the user has just answered a question the agent is blocked on.
 *
 * The previous version asked "are all tools in the last step finished?", which
 * looks reasonable and was catastrophic here: it located the step boundary by
 * searching for a `step-start` part, and Claude Code never emits one. With no
 * boundary the search covered the *entire* message, so every historical tool
 * call — all long since finished — satisfied it, and the client re-sent the
 * same prompt after every single turn. One real conversation ended up with 45
 * copies of the same answer in one assistant message, the agent protesting by
 * the thirteenth that nothing had changed.
 *
 * Requiring the question to be the *last* part is what makes this fire once:
 * as soon as the agent responds, its reply is appended after the answered
 * question and the condition stops holding.
 */
export function shouldAutoSubmit({
  ...
  if (lastPart.type !== "tool-ask_user_question") {
    return false;
  }
