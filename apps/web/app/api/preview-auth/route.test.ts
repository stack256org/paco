...
describe("GET /api/preview-auth", () => {
  ...
  describe("design-candidate hosts inherit the chat's access", () => {
    test("allows a public chat's candidate preview", async () => {
      chat = {
        chatId: "chat-abc",
        ownerUserId: "owner-1",
        visibility: "public",
    ...
    test("denies a private chat's candidate preview for a stranger", async () => {
      chat = {
        chatId: "chat-abc",
        ownerUserId: "owner-1",
        visibility: "private",
      ...
      const response = await GET(
        createRequest({
          forwardedHost: "chat-abc-d1.previews.example.com",
        }),
      );
    ...
    test("allows a private chat's candidate preview for the chat's owner", async () => {
      chat = {
        chatId: "chat-abc",
        ownerUserId: "owner-1",
        visibility: "private",
      ...
      const response = await GET(
        createRequest({
          forwardedHost: "chat-abc-d3.previews.example.com",
        }),
      );
    ...
    test("a grant minted for the candidate host itself still authorizes it", async () => {
      chat = {
        chatId: "chat-abc",
        ownerUserId: "owner-1",
        visibility: "private",
      ...
      const { createPreviewGrantToken } = await grantModulePromise;
      const { token } = createPreviewGrantToken(
        "chat-abc-d2.previews.example.com",
      );
      const { GET } = await routeModulePromise;
