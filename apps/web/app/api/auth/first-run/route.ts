 ...
 * owner. The real guard is the `Origin` check at the top of this handler —
 * Next only applies its own origin check to Server Actions, not to plain
 * route handlers like this one, and passing `headers: request.headers` to
 * `auth.api.*` below (without also passing `request`) means better-auth's
 * `validateOrigin`/`formCsrfMiddleware` never run either, since both
 * early-return when `ctx.request` is unset. Without the check here, a
 * cross-site page could POST here with `enctype="text/plain"` (CORS-safelisted,
 * so no preflight) and claim someone else's fresh install out from under them.
 *
 ...
 * requests racing each other can both pass it. That race is harmless only
 * because of the Origin check above — the window where it matters is exactly
 * the window where the instance is unclaimed, and anyone who can reach this
 * route with a same-origin request could claim the instance anyway.
 *
...
export async function POST(request: Request): Promise<Response> {
  // Missing or `null` counts as a rejection too: a same-origin form post from
