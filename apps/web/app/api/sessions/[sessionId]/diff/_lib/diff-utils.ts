...
/**
 * Largest untracked file rendered into a diff.
 *
 * A file bigger than this is almost never something a person is reviewing — it
 * is a lockfile, a bundle, or a build artefact — and inlining it costs far more
 * than it informs.
 */
const MAX_UNTRACKED_FILE_BYTES = 128 * 1024;

/**
 * Text that Postgres cannot store.
 *
 * `jsonb` holds text, and text cannot contain a NUL byte (SQLSTATE 22P05), so a
 * binary file reaching this point fails the whole diff write rather than just
 * its own entry. Turbopack's on-disk cache is full of such files.
 */
 ...
 * Returns null when there is nothing safe or useful to render.
 */
export function buildUntrackedDiffFile(
  ...
  if (isBinaryContent(content) || content.length > MAX_UNTRACKED_FILE_BYTES) {
    // Listed so the file still shows as added, but without its contents.
    return {
      file: {
        path,
        status: "added",
