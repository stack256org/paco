...
describe("untracked files that broke the server", () => {
  test("does not inline a file containing a NUL byte", () => {
    // Postgres jsonb is text, and text cannot hold \u0000 (SQLSTATE 22P05), so
    // one binary file failed the entire diff write. Turbopack's cache is full of
    // them.
    const result = buildUntrackedDiffFile(
      ".next/dev/cache/turbopack/00000001.sst",
      `some header\u0000\u0000binary payload`,
    );

    expect(result).not.toBeNull();
    expect(result?.lineCount).toBe(0);
    expect(result?.file.diff).not.toContain("\u0000");
    expect(result?.file.diff).toContain("Binary or oversized file not shown");
  });

  test("does not inline an oversized file", () => {
    const result = buildUntrackedDiffFile(
      "package-lock.json",
      "x".repeat(200 * 1024),
    );
  ...
  test("still lists the skipped file as added", () => {
    // Skipping the contents must not hide the file from the review list.
    const result = buildUntrackedDiffFile("big.bin", "x".repeat(200 * 1024));

    expect(result?.file.path).toBe("big.bin");
    expect(result?.file.status).toBe("added");
  });

  test("inlines an ordinary source file unchanged", () => {
    const result = buildUntrackedDiffFile("src/app/page.tsx", "const a = 1;\n");

    expect(result?.lineCount).toBe(1);
    expect(result?.file.diff).toContain("+const a = 1;");
  });
});
