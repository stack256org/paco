  ...
  return {
    ...
    snippets: checkRuns.map((run, index) => ({
      ...
      content: formatSnippetContent(
        run,
        run.id ? compactedLogs[String(run.id)] : undefined,
        run.id ? annotations[String(run.id)] : undefined,
      ),
    })),
  };
}
...
export async function POST(req: Request, context: RouteContext) {
  ...
  if (!sessionRecord.repoOwner || !sessionRecord.repoName) {
    return Response.json(
      { error: "This session isn't connected to a GitHub repository yet." },
      { status: 400 },
    );
  }
  ...
  const runsWithIds = checkRuns.filter(
    (run): run is CheckRun & { id: number } => typeof run.id === "number",
  );
    ...
    await Promise.all(
      runsWithIds.map(async (run) => {
        ...
        const [annotations, rawLog] = await Promise.all([
          ...
          // Logs are plain text, not JSON, so this goes through `gh` directly.
          gh(["api", `repos/${owner}/${repo}/actions/jobs/${run.id}/logs`], {
            token,
          })
