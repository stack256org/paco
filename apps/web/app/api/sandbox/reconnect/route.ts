...
export async function GET(req: Request): Promise<Response> {
  ...
  if (!hasRuntimeSandboxState(sessionRecord.sandboxState)) {
    console.log(
      `[Reconnect] session=${sessionId} status=no_sandbox hasPausedWorkspace=${hasPausedState} runtimeState=false`,
    );
    return Response.json({
      status: "no_sandbox",
      hasPausedWorkspace: hasPausedState,
  ...
  try {
    ...
    console.log(
      `[Reconnect] session=${sessionId} status=connected hasPausedWorkspace=${hasPausedState} expiresAt=${sandbox.expiresAt ?? "null"}`,
    );
    return Response.json({
      status: "connected",
      hasPausedWorkspace: hasPausedState,
    ...
    const hasResumeStateAfterFailure = hasResumableSandboxState(clearedState);
    ...
    console.error(
      `[Reconnect] session=${sessionId} status=expired hasPausedWorkspace=${hasResumeStateAfterFailure} error=${message}`,
    );
    return Response.json({
      status: "expired",
      hasPausedWorkspace: hasResumeStateAfterFailure,
      lifecycle: {
        serverTime: Date.now(),
        state: "hibernated",
        lastActivityAt: null,
