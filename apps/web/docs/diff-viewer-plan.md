...
- Follow pattern from `generate-pr/route.ts` - use `connectSandbox({ sandboxId })`
...
- `createEditDiffLines` from `@paco/shared/lib/diff` - adapt for unified diff parsing
...
- `@paco/sandbox` for `connectSandbox`
- `@paco/shared/lib/diff` for diff utilities
